...
export const shippingCalculate = (options) => {
	return api.post(`${basePath}/shipping-calculate/${options.id}`, options);
};
...
export const getReviews = (options, cancelToken) => {
	...
	return api.get(`${basePath}/reviews/${options.id}${params_qs}`, {cancelToken});
};
...
export const createReview = (options) => {
	return api.post(`${basePath}/reviews/${options.id}`, options);
};
